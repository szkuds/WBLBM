...
class Run:
    ...
    def __init__(self, simulation_type="singlephase", save_interval=100, results_dir="results", **kwargs):
        self.simulation = SimulationFactory.create_simulation(simulation_type, **kwargs)
        self.save_interval = save_interval
        self.results_dir = results_dir
        self.config = self._build_config(simulation_type=simulation_type, save_interval=save_interval, results_dir=results_dir, **kwargs)
        from wblbm.utils.io import SimulationIO
    ...
    def run(self, init_type="standard", verbose=True):
        ...
        if verbose:
            ...
            print(f"Config -> Grid: {self.simulation.grid_shape}, Multiphase: {self.simulation.multiphase}, Wetting: {self.simulation.wetting_enabled}")
        for it in range(nt):
            ...
            if it % self.save_interval == 0 or it == nt - 1:
                ...
                if verbose and hasattr(self.simulation, "macroscopic"):
                    ...
                    if isinstance(result, tuple) and len(result) >= 2:
                        ...
                        print(f"Step {it}/{nt}: avg_rho={avg_rho:.4f}, max_u={max_u:.6f}")
        if verbose:
