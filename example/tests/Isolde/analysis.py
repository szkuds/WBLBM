...
def calculate_droplet_height(rho: jnp.ndarray, rho_mean: float) -> float:
    ...
    if heights:
        ...
        return 0.0
#
# def spherical_cap_radius(height, base_diameter):
#     """Effective radius of curvature for a spherical cap. """
#     height = np.asarray(height)
#     base_diameter = np.asarray(base_diameter)
#
#     a = base_diameter / 2.0
#     Rc = (a**2 + height**2) / (2 * height)
#     Rc[height <= 0] = np.nan
#     return Rc
...
def process_single_simulation(sim_dir: str, output_dir: str) -> str:
    ...
    HALF_NX = NX[0] // 2
    PLOT_EVERY = config.get('save_interval', 1)
    EPS0 = 8.854e-12
    EPS_R = 1
    ...
    X_avg_norm = np.array(avg_x_location_list) / R0
    ...
    global_dir = os.path.join("/Users/isoldeholweg/TUD_LBM/results", "Global_results_10f")
